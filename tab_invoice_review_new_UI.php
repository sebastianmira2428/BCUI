...
<div class="form-group row" style="margin-left:5px;margin-right:5px;">
 ...
 <div class="form-group row" style="margin-left:5px;margin-right:5px;">
  <div class="col-md-12">
    <label class="col-m-2 control-label" id="kulay">Email Approval for Exception</label>
    <input type="text" autocomplete="off" class="form-control" name="email_approval" readonly id="email_approval" style="text-align:center;" value= "">
    <br>
    <input type="button" id="email_approval_copy_button" value="Copy Link" onclick='copyPasteShit("email_approval")'>
    <br>
  </div>
</div>
...
<div class="form-group row" style="margin-left:5px;margin-right:5px;">
  <div class="col-md-12">
    <br>
    <label class="col-m-2 control-label" id="kulay">Invoice Form <i>(e.g. .pdf, .xlsx, .xlsm, .msg)</i></label>
    <input class="form-control" id="w9link" readonly value=""> </input>
    <br>
    <input type="button" id="w9link_copy_button" value="Copy Link" onclick='copyPasteShit("w9link")'>
    <br>
    <br>
    <input class="col-m-2 control-label"  type="file" name="invoice_upload" id="invoice_upload" onChange="validate(this.value)"/>
 ...
 <div class="form-group row" style="margin-left:5px;margin-right:5px;">
  ...
  <div class="col-md-12">
    <br>
    <label class="col-m-2 control-label" id="kulay">Broker Registration Form/ACH <i>(e.g. .pdf, .xlsx, .xlsm, .msg)</i></label>
    <input class="form-control" id="broker" readonly value=""> </input>
    <br>
    <input type="button" id="broker_copy_button" value="Copy Link" onclick='copyPasteShit("broker")'>
    <br>
    <br>
...
<div class="form-group row" id="RejectReasons" hidden style="margin-left:5px;margin-right:5px;">
  ...
  <div class ="col-md-12">
    ...
    <div align="right">
      ...
      <button type="button" class="btn btn-danger inline2" id="rejected" name="rejected ">Reject</button>
      <button type="button" class="btn btn-danger inline2" id="rejected_cancel" name="rejected_cancel">Cancel</button>
    </div>
...
<script>
  ...
  function ToggleAll(source) {
    checkboxes = document.getElementsByName('chkresult[]');
    isToggled = false;

    if(source.checked){
      alert('Warning: Choosing Select all feature will override all records')
    }

    for(var i=0, n=checkboxes.length;i<n;i++) {
      checkboxes[i].checked = source.checked;
      checkbox_clicked(checkboxes[i], true);
    }
  }
  ...
  var checked_array = []
  function checkbox_clicked(checkbox_values, from_Select_all) {
    ...
    checked_array.forEach(function(i){
      ...
      $('#broker_account_code').val(current_checked[10]);
      $('#icp_code').val(current_checked[11]);
      $('#invoice_number').val(current_checked[8]);
      $('#invoice_amount_total').val(current_checked[9]);
      $('#remarks').val(current_checked[12]);
      $('#email_approval').val(current_checked[13]);
      $('#w9link').val(current_checked[14]);
      $('#broker').val(current_checked[15]);

      $('#update_Payment_Currency').val(current_checked[16]);
      $('#update_Commission_Amount').val(current_checked[17]);

      if(document.getElementById("update_Commission_Amount").value == "") {
         document.getElementById("update_Commission_Amount").disabled = true;
         document.getElementById("update_ComAm_checked").checked = false;
      }else{
        document.getElementById("update_Commission_Amount").disabled = false;
        document.getElementById("update_ComAm_checked").checked = true;
      }

      if(document.getElementById("update_Payment_Currency").value == "") {
        document.getElementById("update_Payment_Currency").disabled = true;
        document.getElementById("update_currency_checked").checked = false;

      }else{
        document.getElementById("update_Payment_Currency").disabled = false;
        document.getElementById("update_currency_checked").checked = true;
      }
    ...
    if(isMultiple && !(from_Select_all)){
      hasWarnings = true;
      error_message.push('Multiple Inventory Sale Id detected');
    }

    if(wasReviewed && !(from_Select_all)){
 ...
 $(document).ready(function() {
  ...
  $("#rejected_cancel").click(function(event){
    ...
    $('#update_Commission_Amount').prop('disabled', true);
  });
...
<?php
  ...
  if(isset($_POST['saveDB'])){
    ...
    // echo "<script>consoloe.log($months)</script>"
    ...
    // re enablke for file upload
     ...
     if(isset($_FILES['invoice_upload'])){
       ...
       $newfilename = "Invoice_Form_" . $today . "_ISID-" . $id . "_M-" . $months_for_filename . "_IN-" . $invoice_number . "_RN-" . $Reference_Number1 . "." . $file_ext; // renamed file
   ...
   // $file_ext1 =strtolower(end(explode('.',$_FILES['broker_upload']['name']))); // to get the extension for control
       ...
       $expensions1 = array("pdf","xlsx","xlsm","msg"); // allowed extensions
...
<table class="table table table-hover set-bg" style="margin-left:450px;">
  <thead class="headTable">
   <tr>
    <td style="width:160px">Check the Applicable <br><input class="required_checkboxes" type="checkbox" id="select_all" onclick='ToggleAll(this);'/> Select all</td>
    ...
    <th style="width:450px">Invoice Form</th>
    <th style="width:450px">Broker Registration Form/ACH</th>
  </tr>
</thead>
...
if(isset($_POST['Find'])){
  ...
  if($Reference_Number == ''){
    ...
    /*
    Add this when you enable file path upload


      cd.File_Path_Invoices ,
      cd.File_Path_Email_Approval,
    ...
    */
    ...
    Case when ca.InventorySaleID is not null then 'Cancelled' else '' end as isCancelled
      ...
      LEFT JOIN [BCA].[Cancelled_Deals] ca on cd.InventorySaleID=ca.InventorySaleID
       ...
       order by cd.InventorySaleID, cd.[MTM commission Payment Month]
       ";
    ...
    if($row_count == 0){
      ...
      while($row = $result->fetch(PDO::FETCH_ASSOC)){
        ...
        echo '<tr '.$td_color.'>';
         ...
         echo '<td><label><input name="chkresult[]" '.$row_cancelled.' class="required_checkboxes" type="checkbox" onclick="checkbox_clicked(this)" value="'.$row['saleid'].' ^^ '.$row['Payment_Currency'].' ^^ '.$row['Commission_Amount'].' ^^ '.$row['InventorySaleID'].' ^^ '.$row['MTM commission Payment Month'].' ^^ '.$row['Invoice_Review_By'].' ^^ '.$row['Invoice_Review_Date'].' ^^ '.$row['Update_Remarks'].' ^^ '.$row['invoice_number'].' ^^ '.$row['Invoice_Total_Amount'].' ^^ '.$row['Broker_Account_Code'].' ^^ '.$row['ICP_Code/Business_Unit'].' ^^ '.$row['Review_Remarks'].' ^^ '.$row['File_Path_Email_Approval'].' ^^ '.$row['File_Path_Invoices'].' ^^ '.$row['File_Path_Broker Registration Form/ACH'].' ^^ '.$row['bca_currency'].' ^^ '.$row['bca_commission_amount'].'" </label></td>';

        /*
          Add this on file upload approval

                  echo '<td><label><input name="chkresult[]" class="required_checkboxes" type="checkbox" onclick="checkbox_clicked(this)" value="'.$row['saleid'].' ^^ '.$row['Payment_Currency'].' ^^ '.$row['Commission_Amount'].' ^^ '.$row['InventorySaleID'].' ^^ '.$row['MTM commission Payment Month'].' ^^ '.$row['Invoice_Review_By'].' ^^ '.$row['Invoice_Review_Date'].' ^^ '.$row['Update_Remarks'].' ^^ '.$row['invoice_number'].' ^^ '.$row['Invoice_Total_Amount'].' ^^ '.$row['Broker_Account_Code'].' ^^ '.$row['ICP_Code/Business_Unit'].' ^^ '.$row['Review_Remarks'].' ^^ '.$row['File_Path_Email_Approval'].' ^^ '.$row['File_Path_Invoices'].' ^^ '.$row['File_Path_Broker Registration Form/ACH'].' ^^ '.$row['bca_currency'].' ^^ '.$row['bca_commission_amount'].'" </label></td>';
