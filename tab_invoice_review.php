...
//$row['Broker_Company_Name']
        ...
        foreach($_POST['chkresult'] as $value){
          ...
          array_push($brokercompanyname_array, $comammount[17]);
        ...
        // $brokercompanynames = end($brokercompanyname_array);
        $brokercompanynames =("'".implode( "', '" , $brokercompanyname_array)."'");

        // $brokercompanyname = "'".$brokercompanynames."'";

 // echo $brokercompanynames;
...
<input type="text" id="gettoday" name="gettoday" value="<?php print("$today"); ?>" hidden>
     ...
     <!-- BROKER COMPANY NAME -->
     <input type="text" name="brokercompanynames" id="brokercompanynames" value="<?php print("$brokercompanynames"); ?>" hidden>
...
<?php
  ...
  function rejectCommission($conn, $months, $inventorySaleIds, $db){
    ...
    @$brokercompanyname = $_POST['brokercompanynames'];
    ...
    $query = $conn->query("
      ...
      WHERE [MTM commission Payment Month] in ($months) and InventorySaleID in ($inventorySaleIds) and Broker_Company_Name in ($brokercompanyname);"
    );
...
";
 ...
 echo '<td>'.$row['Reference_ID'].'</td>';
     ...
     echo '<td>'.$row['Broker_Company_Name'].'<input type="text" name="brokercompanyname" hidden value='.$row['Broker_Company_Name'].' ></td>';
