      ...
      <div class="container">
        ...
        <div id="navbar" class="navbar-collapse collapse">
          <ul class="nav navbar-nav">
           ...
           <li class="active"> <?php echo '<a href="home_all.php?user=' . $user . '">Invoice Review</a>'; ?></li>
...
<?php
  ...
  function rejectCommission($conn, $months, $inventorySaleIds){
    ...
    $query = $conn->query("
      Update BCA.TESTCommission_Data_1
      set
    ...
    if($query){
      ...
      saveTransaction($conn, "Invoice Review: Reject Commission for: $Reference_Number1");
    } else {
      echo "<script type='text/javascript'>alert('something went wrong, please try again after a couple of minutes. If the issue persist, please inform the admin')</script>";
    }
  }

  function updateSelectedCommission($conn, $months, $inventorySaleIds, $commission_amount, $invoice_amount_total){
...
function notcommissionable($conn, $months, $inventorySaleIds){
    $review_by = $_POST['review_by'];
    ...
    if($query){
      echo "<script type='text/javascript'>alert('data updated')</script>";
      saveTransaction($conn, "Invoice Review: the Reference Number: $Reference_Number1 is not commissionable");
    } else {
      echo "<script type='text/javascript'>alert('something went wrong, please try again after a couple of minutes. If the issue persist, please inform the admin')</script>";
    }
  }
  ...
  if(isset($_POST['modal_save'])){
    ...
    if($rejected_yes_no == 'Rejected'){
      rejectCommission($conn, $months, $inventorySaleIds);
    } else if ($iscommissionablecheck == 'NOT'){
      notcommissionable($conn,  $months, $inventorySaleIds);
    } else if(is_numeric($invoice_amount_total) && $comAmmountValidation && $currencyValidation){
      updateSelectedCommission($conn, $months, $inventorySaleIds, $commission_amount, $invoice_amount_total);
    } else {
...
<table class="table table table-hover set-bg">
  <thead class="headTable">
   <tr>
    ...
    <th style="width:100px">Exchange Rate</th>
...
if(isset($_POST['Find'])){
  ...
  if($Reference_Number == ''){
    ...
    Case when ca.InventorySaleID is not null then 'Cancelled' else '' end as isCancelled
      ...
      select * from BCA.TESTCommission_Data_1
      where sent = 1 and iscurrent = 1

      UNION

      select distinct a.* from BCA.TESTCommission_Data_1 a
    ...
    LEFT JOIN [BCA].[Cancelled_Deals] ca on cd.InventorySaleID=ca.InventorySaleID
      ...
      where cd.[Reference_Number] = ?");
    $result->execute(array($Reference_Number));
    $result->execute();
   ...
   if($row_count == 0){
    ...
    while($row = $result->fetch(PDO::FETCH_ASSOC)){
     ...
     echo '<td>'.$row['Exchange Rate from GBP to Payment Currency'].'</td>';
